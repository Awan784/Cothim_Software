    ...
    <div class="pb-4">
        <div class="py-4 d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="mb-0">Update expense record.</p>
            </div>
            <div>
            ...
            <form method="post" action="{{ route('expenses.update', $expense) }}">
                ...
                <div class="mb-3">
                    ...
                    <select name="expense_category_id" class="form-select @error('expense_category_id') is-invalid @enderror" required>
                        ...
                        @foreach($expenseCategories as $cat)
                            <option value="{{ $cat->id }}"
                                {{ (string) old('expense_category_id', $expense->expense_category_id) === (string) $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input name="amount" value="{{ old('amount', $expense->amount) }}" class="form-control @error('amount') is-invalid @enderror" required>
                        @error('amount') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Expense Date</label>
                        <x-ams-date-input name="expense_date" :value="$expense->expense_date" required />
                    </div>
                    <div class="col-md-4 mb-3">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="supplier_id" class="form-select @error('supplier_id') is-invalid @enderror">
                            ...
                            @foreach($suppliers as $s)
                                <option value="{{ $s->id }}"
                                    {{ (string) old('supplier_id', $expense->supplier_id) === (string) $s->id ? 'selected' : '' }}>
                                    {{ $s->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="bank_account_id" class="form-select @error('bank_account_id') is-invalid @enderror">
                            ...
                            @foreach($bankAccounts as $b)
                                <option value="{{ $b->id }}"
                                    {{ (string) old('bank_account_id', $expense->bank_account_id) === (string) $b->id ? 'selected' : '' }}>
                                    {{ $b->name }}
                                </option>
                            @endforeach
                        </select>
